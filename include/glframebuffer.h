...
namespace lix
{
    ...
    public:
        ...
        void createRenderBuffer(GLenum attachment=GL_DEPTH_STENCIL_ATTACHMENT, GLenum internalFormat=GL_DEPTH24_STENCIL8);
        void createRenderBufferMultisample(GLenum attachment=GL_COLOR_ATTACHMENT0, GLenum internalFormat=GL_RGBA8, GLsizei samples=4);

        void attach(GLenum attachment, std::shared_ptr<RenderBuffer> renderBuffer);

        std::shared_ptr<lix::Texture> createTexture(GLuint colorAttachment=GL_COLOR_ATTACHMENT0, GLuint internalFormat=GL_RGB, GLuint format=GL_RGB, GLuint type=GL_UNSIGNED_BYTE);

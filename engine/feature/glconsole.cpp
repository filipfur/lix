...
bool lix::Console::onMouseDown(KeySym key, KeyMod mod) {
    ...
    {
        lix::Color& color = std::dynamic_pointer_cast<color_command>(executedCommand)->color();
...
bool lix::Console::onMouseMove(float x, float y, float xrel, float yrel) {
    ...
    if(colorMode != ColorMode::NONE) {
        ...
        switch(colorMode) {
            case ColorMode::RED:
                text = std::to_string((int)glm::round(tweakFloat(color.vec4().r, factor) * 255));
                break;
            case ColorMode::GREEN:
                text = std::to_string((int)glm::round(tweakFloat(color.vec4().g, factor) * 255));
                break;
            case ColorMode::BLUE:
                text = std::to_string((int)glm::round(tweakFloat(color.vec4().b, factor) * 255));
                break;
            case ColorMode::YELLOW:
                text = std::to_string((int)glm::round(std::max(tweakFloat(color.vec4().r, factor), tweakFloat(color.vec4().g, factor)) * 255));
                break;
            case ColorMode::MAGENTA:
                text = std::to_string((int)glm::round(std::max(tweakFloat(color.vec4().r, factor), tweakFloat(color.vec4().b, factor)) * 255));
                break;
            case ColorMode::CYAN:
                text = std::to_string((int)glm::round(std::max(tweakFloat(color.vec4().g, factor), tweakFloat(color.vec4().b, factor)) * 255));
                break;
            case ColorMode::HUE:
                color = intermediateColor;
                H = std::max(-glm::pi<float>(), std::min(glm::pi<float>(), H + factor));
                color.hueShift(H);
...
void lix::Console::updateText()
{
    textNode->setText(label + prefix + text);
}
...
bool lix::Console::onKeyDown(KeySym key, KeyMod mod) {
    ...
    if(colorMode != ColorMode::NONE)
    {
        ...
        case SDLK_ESCAPE:
            colorMode = ColorMode::NONE;
            std::dynamic_pointer_cast<color_command>(executedCommand)->color().vec4() = restoreColor.vec4();
            close();
            break;
        case SDLK_r:
            colorMode = (mod & KMOD_SHIFT) ? ColorMode::CYAN : ColorMode::RED;
            text = std::to_string((int)glm::round(std::dynamic_pointer_cast<color_command>(executedCommand)->color().vec4().r * 255));
            updatePrefix();
            updateText();
            break;
        case SDLK_g:
            colorMode = (mod & KMOD_SHIFT) ? ColorMode::MAGENTA : ColorMode::GREEN;
            text = std::to_string((int)glm::round(std::dynamic_pointer_cast<color_command>(executedCommand)->color().vec4().g * 255));
            updatePrefix();
            updateText();
            break;
        case SDLK_b:
            colorMode = (mod & KMOD_SHIFT) ? ColorMode::YELLOW : ColorMode::BLUE;
            text = std::to_string((int)glm::round(std::dynamic_pointer_cast<color_command>(executedCommand)->color().vec4().b * 255));
            updatePrefix();
            updateText();
            break;
        case SDLK_h:
...
bool lix::Console::onKeyUp(KeySym, KeyMod) {
    return false;
}
    ...
    for(const auto& c : commands)
    {
        if(starts_with(cmd.c_str(), c->shortname))
        {
            ...
            if(c->perform(*this, cmd))
            {
                if(colorMode != ColorMode::NONE) {
                    updatePrefix();
                    restoreColor = std::dynamic_pointer_cast<color_command>(c)->color();
                    intermediateColor = restoreColor;
                    auto hsv = restoreColor.hsv();
                    H = hsv[0];
                    S = hsv[1];
                    V = hsv[2];
                    text = std::to_string((int)glm::round(restoreColor.vec4().r * 255));

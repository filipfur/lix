...
lix::Timer::Timer(Time::Raw duration) : _duration{duration}, _active{true}, _expired{Time::raw() + duration}
{}

bool lix::Timer::active() const
{
    return _active;
}

lix::Time::Raw lix::Timer::timeLeft() const
{
    return _expired - Time::raw();
}
...
bool lix::Timer::elapsed() const
{
    return _active && Time::raw() >= _expired;
}

...
lix::Shader::Shader(GLenum type) { _id = glCreateShader(type); }

lix::Shader::~Shader() noexcept { glDeleteShader(_id); }
...
void lix::Shader::compile(const char *src) {
    ...
    if (status == GL_FALSE) {
        ...
        std::string str{src};
        std::cerr << "'" << _id << "' '" << str
                  << "' compiled with error:" << std::endl;
        std::copy(errorLog.begin(), errorLog.end(),
                  std::ostream_iterator<GLchar>(std::cout, ""));
        exit(1);
    }
}
...
lix::ShaderProgram::ShaderProgram(const char *vertexSrc, const char *fragSrc)
    : _vertexShader{GL_VERTEX_SHADER}, _fragmentShader{GL_FRAGMENT_SHADER} {
    _id = glCreateProgram();
...
lix::ShaderProgram::~ShaderProgram() noexcept { glDeleteProgram(_id); }
...
GLuint lix::ShaderProgram::loadUniform(const std::string &name) {
    ...
    if (it == _uniforms.end()) {
        ...
        if (id == INVALID_LOCATION) {
            std::cerr << "Failed to load uniform '" << name << "' inside "
                      << _id << std::endl;
        }
...
lix::ShaderProgram *lix::ShaderProgram::setUniform(const std::string &name,
                                                   GLfloat f) {
...
lix::ShaderProgram *
lix::ShaderProgram::setUniform(const std::string &name,

...
void lix::Buffer::bufferDataInternal(GLuint byteLength, GLuint stride, void* data)
{
    assert(stride != 0);
...
void lix::Buffer::bufferData(const std::vector<GLuint>& data)
{
    ...
    bufferDataInternal(static_cast<GLuint>(data.size() * stride), static_cast<GLuint>(stride), (void*)data.data());

...
lix::FrameBuffer::~FrameBuffer() noexcept { glDeleteFramebuffers(1, &_id); }
...
void lix::FrameBuffer::attach(GLenum attachment,
...
void lix::FrameBuffer::createRenderBuffer(GLenum attachment,
...
void lix::FrameBuffer::createRenderBufferMultisample(GLenum attachment,
                                                     GLenum internalFormat,
                                                     GLsizei samples) {
    auto renderBuffer = std::make_shared<lix::RenderBuffer>();
    renderBuffer->bind();
...
void lix::FrameBuffer::readPixels(GLint x, GLint y, GLsizei width,
                                  GLsizei height, GLenum format, GLenum type,
                                  void *data) {
    glReadPixels(x, y, width, height, format, type, data);
}

void lix::FrameBuffer::readPixel(GLint x, GLint y, GLenum format, GLenum type,
                                 void *data) {
...
lix::FrameBuffer::texture(GLuint colorAttachment) {
    return _textures.at(colorAttachment);
}

glm::ivec2 lix::FrameBuffer::resolution() const { return _resolution; }

std::shared_ptr<lix::RenderBuffer>
lix::FrameBuffer::renderBuffer(size_t i) const {
...
void lix::FrameBuffer::blit(std::shared_ptr<lix::FrameBuffer> toFrameBuffer,
                            ...
                            GLuint toComponent, GLbitfield mask,
                            GLenum filter) {
    bindAsReadBuffer();
    if (toFrameBuffer) {
...
void lix::FrameBuffer::bindTexture(GLuint colorAttachment, GLuint textureUnit) {
    ...
    if (it != _textures.end()) {
        ...
        std::cerr << "No texture id found for color attachment: "
                  << colorAttachment << std::endl;
    }
}
...
void lix::FrameBuffer::bindColorsAsDrawBuffers() {
    ...
    for (auto it = _textures.begin(); it != _textures.end(); ++it) {
        if (it->first >= GL_COLOR_ATTACHMENT0 &&
            it->first <= GL_COLOR_ATTACHMENT15) {
...
const char *lix::FrameBuffer::getFramebufferStatusString(GLenum status) {
    ...
    case GL_FRAMEBUFFER_UNDEFINED:
        return "GL_FRAMEBUFFER_UNDEFINED: The framebuffer object is not "
               "complete because it has no defined buffers.";
    case GL_FRAMEBUFFER_INCOMPLETE_ATTACHMENT:
        return "GL_FRAMEBUFFER_INCOMPLETE_ATTACHMENT: The framebuffer object "
               "is not complete because at least one of its buffer attachments "
               "is incomplete.";
    case GL_FRAMEBUFFER_INCOMPLETE_MISSING_ATTACHMENT:
        return "GL_FRAMEBUFFER_INCOMPLETE_MISSING_ATTACHMENT: The framebuffer "
               "object is not complete because it has no attached buffers.";
    /*case GL_FRAMEBUFFER_INCOMPLETE_DRAW_BUFFER:
        return "GL_FRAMEBUFFER_INCOMPLETE_DRAW_BUFFER: The framebuffer object "
               "is not complete because the value of "
               "GL_FRAMEBUFFER_ATTACHMENT_OBJECT_TYPE is GL_NONE for the color "
               "attachment point named by GL_DRAW_BUFFERi.";*/
    /*case GL_FRAMEBUFFER_INCOMPLETE_READ_BUFFER:
        return "GL_FRAMEBUFFER_INCOMPLETE_READ_BUFFER: The framebuffer object "
               "is not complete because the value of "
               "GL_FRAMEBUFFER_ATTACHMENT_OBJECT_TYPE is GL_NONE for the color "
               "attachment point named by GL_READ_BUFFER.";*/
    case GL_FRAMEBUFFER_UNSUPPORTED:
        return "GL_FRAMEBUFFER_UNSUPPORTED: The combination of internal "
               "formats of the attached images violates an "
               "implementation-dependent set of restrictions.";
    case GL_FRAMEBUFFER_INCOMPLETE_MULTISAMPLE:
        return "GL_FRAMEBUFFER_INCOMPLETE_MULTISAMPLE: The value of "
               "GL_RENDERBUFFER_SAMPLES is not the same for all attached "
               "renderbuffers; the value of GL_TEXTURE_SAMPLES is the not same "
               "for all attached textures; or the attached images are a mix of "
               "renderbuffers and textures, the value of "
               "GL_RENDERBUFFER_SAMPLES does not match the value of "
               "GL_TEXTURE_SAMPLES, and at least one of the attachments is a "
               "texture.";
    default:
...
void lix::FrameBuffer::checkStatus() {
    ...
    if (status != GL_FRAMEBUFFER_COMPLETE) {
        std::cerr << "Error: FBO " << getFramebufferStatusString(status)
                  << std::endl;
    }
}
...
namespace lix {
struct Half_Edge {
    Half_Edge(const glm::vec3 &vertex_, struct Face *face_)
        : vertex{vertex_}, face{face_} {
...
struct Face {
    ...
    glm::vec3 center() {
        return (half_edge->vertex + half_edge->next->vertex +
                half_edge->next->next->vertex) *
               0.333333f;
    }

...
bool lix::gjk2d(Shape &shapeA, Shape &shapeB, std::vector<glm::vec3> &simplex,
    ...
    // Check if the CO and OB are pointing in the same "general" direction.
    // Otherwise the origin was not crossed.
    if (glm::dot(BC, BO) < 0) // CO dot OB. From the perspective of C is point B
                              // further away than the origin.
    {

...
lix::gjk_state emptyCase(lix::Shape &shapeA, lix::Shape &shapeB, std::vector<glm::vec3> &simplex, lix::Collision *, const glm::vec3 &initialDirection)
{
    ...
    if(glm::dot(-A, -B) > 0)
    {
        return lix::gjk_state::NO_COLLISION; // TODO: Should try more start directions of D.
    }
    ...
    logOfs << "shapeA " << shapeA.trs()->translation()
        << " {" << shapeA.trs()->rotation().w
        << ' ' << shapeA.trs()->rotation().x
        << ' ' << shapeA.trs()->rotation().y
        << ' ' << shapeA.trs()->rotation().z
        << "} " << shapeA.trs()->scale() << std::endl;
    logOfs << "shapeB " << shapeB.trs()->translation()
        << " {" << shapeB.trs()->rotation().w
        << ' ' << shapeB.trs()->rotation().x
        << ' ' << shapeB.trs()->rotation().y
        << ' ' << shapeB.trs()->rotation().z
        << "} " << shapeB.trs()->scale() << std::endl;
    ...
    glm::vec3 supportOfC = shapeA.supportPoint(initialDirection) - shapeB.supportPoint(-initialDirection);
...
lix::gjk_state edgeCase(lix::Shape &shapeA, lix::Shape &shapeB, std::vector<glm::vec3> &simplex, lix::Collision *collision)
{
...
#ifdef COLLISION_LOG_TO_FILE
    logOfs << "edgeCase " << simplex.at(0) << " " << simplex.at(1) << " " << simplex.at(2) << std::endl;
#endif

    return lix::gjk_state::INCREMENTING;
}

lix::gjk_state triangleCase(lix::Shape &shapeA, lix::Shape &shapeB, std::vector<glm::vec3> &simplex)
{
...
#ifdef COLLISION_LOG_TO_FILE
    logOfs << "triangleCase " << simplex.at(0) << " " << simplex.at(1) << " " << simplex.at(2) << " " << simplex.at(3) << std::endl;
#endif

    return lix::gjk_state::INCREMENTING;
}

lix::gjk_state tetrahedronCase(lix::Shape & shapeA, lix::Shape & shapeB, std::vector<glm::vec3> &simplex, lix::Collision *collision)
{
...
#ifdef COLLISION_LOG_TO_FILE
    logOfs << "tetrahedronCase " << simplex.at(0) << " " << simplex.at(1) << " " << simplex.at(2) << " " << simplex.at(3) << std::endl;
#endif
...
lix::gjk_state lix::gjk_increment(Shape &shapeA, Shape &shapeB, std::vector<glm::vec3> &simplex,
                                  const glm::vec3 &initialDirection, lix::Collision *collision)
{
...
{
    float minDistance = glm::dot(ch.begin()->normal, ch.begin()->half_edge->vertex);
...
bool lix::epa(Shape &shapeA, Shape &shapeB, const std::vector<glm::vec3> &simplex, lix::Collision *collision)
{
...
bool lix::epa_increment(Shape &shapeA, Shape &shapeB, lix::ConvexHull &ch, lix::Collision *collision)
{
    auto [minFaceIt, minDistance] = findClosestFace(ch);
    glm::vec3 sp = minkowskiSupportPoint(shapeA, shapeB, minFaceIt->normal); // shapeA.supportPoint(minFaceIt->normal) - shapeB.supportPoint(-minFaceIt->normal);
...
glm::vec3 lix::getSupportPointOfA(lix::Shape& shape, const glm::vec3& minkowskiSP)
{
    return shape.supportPoint(minkowskiToDirection.at(minkowskiSP));
}

glm::vec3 lix::getSupportPointOfB(lix::Shape& shape, const glm::vec3& minkowskiSP)
{
    return shape.supportPoint(-minkowskiToDirection.at(minkowskiSP));
}
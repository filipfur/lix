...
bool popAlongDirection(std::vector<glm::vec3> &s, const glm::vec3 &D,
                       glm::vec3 &rval) {
...
void checkFace(std::set<lix::Face *> &visible, lix::Face *face,
               const glm::vec3 &p) {
    ...
    for (auto &face : faces) {
        ...
        do {
            printf(" <%d> next: <%d>, opposite: <%d> f%d\n", e->id, e->next->id,
                   e->opposite ? e->opposite->id : -1,
                   e->opposite ? e->opposite->face->id : -1);
            if (e->opposite) {
...
/*auto toVertexVector(const std::vector<glm::vec3>& vector)
{
    ...
    uint32_t index{0};
    std::transform(vector.begin(), vector.end(), rval.begin(), [&index](const
glm::vec3& v) -> lix::Vertex { return {index++, v};
    });
    return rval;
}
...
lix::ConvexHull::ConvexHull(const std::vector<glm::vec3> &points) {
    ...
    assert(!lix::isSameVertex(A, D) && !lix::isSameVertex(B, D) &&
           !lix::isSameVertex(C, D));
...
bool lix::ConvexHull::addPoint(const glm::vec3 &p, lix::Face *face,
                               std::list<Face *> *Q) {
    if (face == nullptr) {
    ...
    while (fIt != visible.end()) {
        ...
        if (Q)
            Q->remove(f);
        _faces.remove_if([f](const auto &face) { return f->id == face.id; });
...
void lix::ConvexHull::addPoints(std::vector<glm::vec3> &P) {
    std::list<Face *> Q;
    std::for_each(std::begin(_faces), std::end(_faces),
                  [&Q](Face &face) { Q.push_back(&face); });
...
void lix::ConvexHull::connect(Half_Edge *self, Half_Edge *next,
                              Half_Edge *opposite, Face *face) {
    self->next = next;
    next->prev = self;
...
lix::ConvexHull::rayIntersect(const glm::vec3 &ray_origin,

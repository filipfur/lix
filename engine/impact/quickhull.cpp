...
float distanceFromLine(const glm::vec3 &a, const glm::vec3 &b,
    ...
    return glm::abs((b.x - a.x) * (a.y - p.y) - (a.x - p.x) * (b.y - a.y)) /
           glm::sqrt((b.x - a.x) * (b.x - a.x) + (b.y - a.y) * (b.y - a.y));
};

float signDistanceFromLine(const glm::vec3 &a, const glm::vec3 &b,
                           const glm::vec3 &p) {
    // TODO: Optimize (not needing actual distance)
    // TODO: Cache results since line isn't moving.
...
float magnitudeFromLine(const glm::vec3 &a, const glm::vec3 &b,
                        const glm::vec3 &p) {
    return glm::abs((b.x - a.x) * (a.y - p.y) - (a.x - p.x) * (b.y - a.y));
};
...
void findHull(const std::vector<glm::vec3> &s, glm::vec3 p, glm::vec3 q,
              std::vector<glm::vec3> &CH) {
    if (s.empty()) {
        return;
    }
    float maxDistance{0};
...
void lix::quickHull(const std::vector<glm::vec3> &s,
                    std::vector<glm::vec3> &ch) {
    std::vector<glm::vec3> s1;
    std::vector<glm::vec3> s2;
    ...
    std::sort(ch.begin(), ch.end(),
              [&center](const glm::vec3 &a, const glm::vec3 &b) -> bool {
...
void lix::quick_hull(const std::vector<glm::vec3> &points,
    ...
    while (!Q.empty()) {
        ...
        for (unsigned int visible : visibleFaces) {
            for (unsigned int invisible : invisibleFaces) {
                Edge e;
                if (lix::isAdjacent(&faces[visible * 3], &faces[invisible * 3],
                                    e)) {
                }
            }
        }

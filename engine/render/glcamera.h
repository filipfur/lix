...
class Camera : public lix::Node {
  public:
    ...
    void setupUBO(const std::vector<lix::Element *> &shaders) {
        _ubo.reset(new lix::UniformBuffer(sizeof(CameraBlock), (void *)&_block,
                                          "CameraBlock", 0, GL_STATIC_DRAW));
    ...
    void refresh(float dt) {
        ...
        if (glm::length(d) > 0) {
            _block.position =
                glm::mix(_block.position, _targetPosition, 4.0f * dt);
            positionUpdated = true;
        }

        if (_viewInvalid || positionUpdated) {
            _ubo->bufferSubData(sizeof(glm::mat4),
                                sizeof(glm::mat4) +
                                    sizeof(glm::vec3)); // view + position
            _viewInvalid = false;
        }
    }

    const glm::vec3 &position() const { return _block.position; }

    glm::vec3 forward() {
        return glm::vec3{_block.view[0][0], _block.view[0][1],
                         _block.view[0][2]};
    }

    glm::vec3 up() {
        return glm::vec3{_block.view[1][0], _block.view[1][1],
                         _block.view[1][2]};
    }

    glm::vec3 right() {
        return glm::vec3{_block.view[2][0], _block.view[2][1],
                         _block.view[2][2]};
    }

...
namespace lix
{
    ...
    {
        static lix::Attributes attributes()
        {
            return {lix::Attribute::MAT4};
        }
    ...
    template <typename Container, typename Allocator=lix::ModelAllocator<typename Container::value_type>> 
    class InstancedRendering
    {
    public:
        InstancedRendering(MeshPtr mesh, Container& instances, GLenum usage=GL_STATIC_DRAW) : _mesh{mesh}, _instances{instances}
        {
            ...
            _instancesVBO = vao->createVbo(usage, Allocator::attributes(), nullptr, 0, 1);

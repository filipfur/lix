...
void procPlyFile(fs::path filePath, fs::path outputDir) {
    ...
    std::copy(vertexBuffer.data, vertexBuffer.data + vertexBuffer.data_size,
              std::back_inserter(buf));
    std::copy(indexBuffer.data, indexBuffer.data + indexBuffer.data_size,
              std::back_inserter(buf));
    ...
    { // export .cpp
        ...
        std::string delim{""};
        size_t offset{0};
        for (auto bufferIt = std::begin(buffers); bufferIt != std::end(buffers);
             ++bufferIt) {
            ofs << delim;
            delim = ",\n";

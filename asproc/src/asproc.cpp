...
int main(int argc, const char* argv[])
{
    ...
    for(int i{1}; i < argc; ++i)
    {
        if(strcmp(argv[i], "-s") == 0) {
            if(i + 2 >= argc) {
                std::cerr << "Failed to parse arguments for option -s" << std::endl;
                usage();
                return 1;
            }
            mode = Mode::SHADERS;
            ...
            if(i + 2 >= argc) {
                std::cerr << "Failed to parse arguments for option -i" << std::endl;
                usage();
                return 1;
            }
            mode = Mode::IMAGES;
        ...
        else if(strcmp(argv[i], "--convert-to-srgb") == 0)
        {
            ...
            if(i + 2 >= argc) {
                std::cerr << "Failed to parse arguments for option -o" << std::endl;
                usage();
                return 1;
            }
            mode = Mode::OBJECTS;
            ...
            if(i + 1 >= argc) {
                std::cerr << "Failed to parse arguments for option --version-override" << std::endl;
                usage();
                return 1;
            }
            ...
            if(i + 2 >= argc) {
                std::cerr << "Failed to parse arguments for option -f" << std::endl;
                usage();
                return 1;
            }
            mode = Mode::FONTS;

    ...
    const gltf::Node& node)
{
    ...
    ofs << "const gltf::Node " << scope << nodeName(node) << "{\""
        << node.name << "\", "
...
void exportNodeChildren(std::ofstream& ofs, const std::string& scope, const gltf::Node& node)
{
...
{
    ofs << "static const unsigned char " << textureName(texture) << "_data[] = {\n"; 
    ...
    ofs << "const gltf::Texture " << scope << textureName(texture) << "{\""
                            ...
                            << texture.width << ", "
                            << texture.height << ", "
                            << texture.channels << ", " << textureName(texture) << "_data};\n";
}
...
{
    ofs << "const gltf::Channel " << animationName(animation) << "_channels[] = {\n";
    std::string delim{""};
    ...
    ofs << "const gltf::Animation " << scope << animationName(animation) << "{"
        << std::quoted(animation.name) << ", " << animationName(animation) << "_channels, " << animation.channels_size << " \n};\n";
}
...
void exportMesh(std::ofstream& ofs, const std::string& scope, const gltf::Mesh& mesh)
{
    ...
    for(size_t i{0}; i < mesh.primitives_size; ++i)
    {
        ...
        ofs << "static const gltf::Buffer* " << meshName(mesh) << "_prim" << i << "_attr" << "[] = {\n";
        std::string delim{""};
    ...
    for(size_t i{0}; i < mesh.primitives_size; ++i)
    {
        ...
        ofs << primitiveDelim << "\n    {\n        "
                ...
                << ",\n";
        ofs << "        " << meshName(mesh) << "_prim" << i << "_attr, " << primitive.attributes_size << ", \n";
...
void exportSkin(std::ofstream& ofs, const std::string& scope, const gltf::Skin& skin)
{
    ...
    ofs << "const gltf::Skin " << scope << skinName(skin) << "{" << std::quoted(skin.name) << ", "

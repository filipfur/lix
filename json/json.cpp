...
json::Json::Json(const json::Type &type, const std::string &key,
                 const std::string &value)
    : _type{type}, _key{key}, _value{value} {}

json::Json::~Json() noexcept { clear(); }
...
enum class State { ParseKey, ParseValue, ParseElements };
...
void json::Json::parse(std::istream &is, json::Json &obj) {
    ...
    auto handleStart = [&](char start, char end) {
        if (startchar == 0) {
            ...
            json::Json anotherOne{start == '{' ? json::Object : json::Array,
                                  key, ""};
        ...
        if (parseString) {
            ...
            case ',':
                ...
                case State::ParseElements: {
                    if (stringValue.length() >
                        0) // Probably already handled in string parsing.
                    {
            ...
            case ']':
                ...
                case State::ParseElements:
                    ...
                    break;
                case State::ParseKey:
                    // throw std::runtime_error("unexpected endchar during
                    // ParseKey.");
                    break;
                }
                // is.get();
...
std::istream &json::operator>>(std::istream &is, json::Json &obj) {
    ...
    try {
        ...
        std::cout << "Error at line " << ++__lineCount << ": " << e.what()
                  << std::endl;
    }
    return is;
}
